    ...
    <div className="max-w-2xl mx-auto space-y-5 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            Attach files to a forensic case
          </p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-white/3 border border-gray-100 dark:border-white/5 rounded-2xl p-6 space-y-5">
        ...
        <div>
          ...
          {cases.length === 0 && (
            <p className="text-xs text-gray-400 mt-1.5">
              No cases yet.{" "}
              <Link href="/cases/new" className="text-blue-600 dark:text-blue-400 hover:underline">
                Create one first.
              </Link>
            </p>
          )}
        ...
        <button
          ...
          disabled={uploading || !files.length || !caseId}
        ...
        >
          {uploading ? (
            ...
            <>
              ⬆️ Upload {files.length > 0 ? `${files.length} File${files.length > 1 ? "s" : ""}` : "Evidence"}
            </>
          )}
        </button>
      </div>
      ...
      {uploaded.length > 0 && (
        <div className="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-2xl p-5">
          ...
          {caseId && (
            <Link
              href={`/cases/${caseId}`}
              className="text-xs text-blue-600 dark:text-blue-400 hover:underline mt-3 block font-medium"
            >
              View case →
            </Link>
          )}
